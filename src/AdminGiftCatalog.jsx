...
import { ExternalLink, Gift, Pencil, Plus, Power, PowerOff, RotateCcw, Trash2, X } from 'lucide-react';
...
export default function AdminGiftCatalog({ catalog, reservations, onRefresh, onNotice, onError }) {
  ...
  return <section className="admin-section admin-catalog"><div className="admin-section-title"><div><p className="eyebrow">CATÁLOGO</p><h2>Gerenciar presentes</h2><p>Edite títulos e links, controle a visibilidade e faça reservas manuais.</p></div><button className="button primary" onClick={() => setEditing({ number: nextNumber, category: 'Nosso cantinho', store: 'Shopee', active: true, deleted: false })}><Plus size={16}/>Novo presente</button></div>
    <div className="catalog-admin-list">{catalog.map(gift => {
    ...
    {reserving && <div className="reservation-overlay"><section className="reservation-dialog" role="dialog" aria-modal="true" aria-labelledby="admin-reserve-title"><button className="close-reservation" aria-label="Fechar" onClick={() => setReserving(null)} disabled={busy}><X/></button><form onSubmit={reserve}><h3 id="admin-reserve-title">Reservar pelo painel</h3><p className="reservation-product">#{String(reserving.number).padStart(2, '0')} · {reserving.name}</p><label>Nome do convidado<input autoFocus name="name" minLength={2} maxLength={100} required/></label><label>WhatsApp com DDD<input name="phone" type="tel" pattern="[0-9 ()+\-]{10,20}" maxLength={20} required/></label><button className="button primary w-full justify-center" disabled={busy}>{busy ? 'Reservando…' : 'Confirmar reserva'}</button></form></section></div>}
  </section>;
}
